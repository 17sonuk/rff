...
const invokeChaincode = async function (peerNames, channelName, chaincodeName, fcn, args, username, org_name) {
	...
	try {
		...
		if (all_good) {
			...
			logger.debug('Total eventhubs: ' + event_hubs.length)
			...
			event_hubs.forEach((eh) => {
				...
				let invokeEventPromise = new Promise((resolve, reject) => {
					...
					eh.registerTxEvent(tx_id_string, (tx, code, block_num) => {
						...
						eh.registerChaincodeEvent('csrcc', 'Notification', (ccEventObj, blockNum, txId, txStatus) => {
							logger.info('bc tx id: ' + txId)
							logger.info('blocknumber: ' + blockNum)
							var payload = String.fromCharCode(...ccEventObj.payload)
							...
							if (txStatus !== 'VALID') {
								...
								let message = 'The invoke chaincode transaction was valid. ' + payload

								//TODO: Store the notification details in mongo.
								var notificationObj = JSON.parse(payload)
							...
							clearTimeout(event_timeout);
							logger.error("Failed obtaining chaincode event due to: " + err)
							reject(err)
						}, { unregister: true, disconnect: true })
					}, (err) => {
						clearTimeout(event_timeout);
						logger.error("Failed obtaining transaction event due to: " + err)
						reject(err);
					});
					eh.connect(true);
				});
	...
	if (error_message) {
		...
		success = false;
		//Workaround for ChannelEventHub error. Forcefully send success.
		if (error_message.includes("ChannelEventHub has been shutdown")) {
			success = true;
		}
		logger.error(message);
	} else {
