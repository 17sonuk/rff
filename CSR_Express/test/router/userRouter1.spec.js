...
describe('USER ROUTER - CHECKUSERNAME API', () => {
    ...
    it('testing checkUserNameValidity API', async function () {
        mockObj.resolves("User already exists")
        const response = await request(app)
            .post("/mongo/user/checkUserNameValidity")
            .send({ 
                userName: "ngo2",
            });
        console.log("Response in checkUserNameValidity API", response.)
        console.log("Response in checkUserNameValidity API", response.data)
        console.log("Response in checkUserNameValidity API", response.success)
