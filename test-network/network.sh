...
function createOrgs() {
  ...
  #while :
   # do
    ...
    infoln "Creating Corporate Identities"

    createCorporate

    infoln "Creating Credits Authority Identities"

    createCreditsAuthority

    infoln "Creating Ngo Identities"
  ...
  sudo chmod -R 775 ./organizations
  infoln "Generating CCP files for Corporate, Credits Authority  and Ngo"
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile ThreeOrgsCsrOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/corporate/msp organizations/fabric-ca/corporate/tls-cert.pem organizations/fabric-ca/corporate/ca-cert.pem organizations/fabric-ca/corporate/IssuerPublicKey organizations/fabric-ca/corporate/IssuerRevocationPublicKey organizations/fabric-ca/corporate/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ngo/msp organizations/fabric-ca/ngo/tls-cert.pem organizations/fabric-ca/ngo/ca-cert.pem organizations/fabric-ca/ngo/IssuerPublicKey organizations/fabric-ca/ngo/IssuerRevocationPublicKey organizations/fabric-ca/ngo/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/creditsauthority/msp organizations/fabric-ca/creditsauthority/tls-cert.pem organizations/fabric-ca/creditsauthority/ca-cert.pem organizations/fabric-ca/creditsauthority/IssuerPublicKey organizations/fabric-ca/creditsauthority/IssuerRevocationPublicKey organizations/fabric-ca/creditsauthority/fabric-ca-server.db'
